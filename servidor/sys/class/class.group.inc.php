...
class group extends db_connect
{
    ...
    public function create2($group_name, $group_fullname, $group_category, $group_desc, $group_site, $group_location, $year, $month, $day, $group_allow_posts, $group_allow_comments, $finalidad)
    {

        $result = array("error" => true);

        $helper = new helper($this->db);
        $group_name = str_replace(' ', '-', $group_fullname);
        ...
        $latlong = $this->get_lat_long($group_location); // create a function with the name "get_lat_long" given as below
        //$latlong = "36.4,4.13";
        //$map = explode(',' ,$latlong);
        $mapLat = $latlong[0];
        $mapLong = $latlong[1];
        $mapAddress = $latlong[2];


        $stmt = $this->db->prepare("INSERT INTO users (state, login, fullname, account_author, account_type, account_category, status, country, my_page, allowComments, allowPosts, bYear, bMonth, bDay, regtime, ip_addr, lat, lng, finalidad) value (:state, :username, :fullname, :account_author, :account_type, :account_category, :status, :country, :my_page, :allowComments, :allowPosts, :bYear, :bMonth, :bDay, :createAt, :ip_addr, :lat, :lng, :finalidad)");
        $stmt->bindParam(":state", $accountState, PDO::PARAM_INT);
        $stmt->bindParam(":username", $group_name, PDO::PARAM_STR);
        $stmt->bindParam(":fullname", $group_fullname, PDO::PARAM_STR);
        $stmt->bindParam(":account_author", $this->requestFrom, PDO::PARAM_INT);
        ...
        $stmt->bindParam(":account_category", $group_category, PDO::PARAM_INT);
        $stmt->bindParam(":status", $group_desc, PDO::PARAM_STR);
        $stmt->bindParam(":country", $mapAddress, PDO::PARAM_STR);
        $stmt->bindParam(":my_page", $group_site, PDO::PARAM_STR);
        $stmt->bindParam(":allowComments", $group_allow_comments, PDO::PARAM_INT);
        $stmt->bindParam(":allowPosts", $group_allow_posts, PDO::PARAM_INT);
        $stmt->bindParam(":bYear", $year, PDO::PARAM_INT);
        ...
        $stmt->bindParam(":createAt", $currentTime, PDO::PARAM_INT);
        $stmt->bindParam(":ip_addr", $ip_addr, PDO::PARAM_INT);
        $stmt->bindParam(":lat", $mapLat, PDO::PARAM_INT);
        $stmt->bindParam(":lng", $mapLong, PDO::PARAM_INT);
        $stmt->bindParam(":finalidad", $finalidad, PDO::PARAM_STR);
    ...
    $json = file_get_contents("https://maps.google.com/maps/api/geocode/json?address=$address&key=AIzaSyAls6F286BpRLxIJVWP7dkUEpoYcmJXrOU");
    ...
    public function setLocation($location)
    {
        ...
        $mapLat = $latlong[0];
        $mapLong = $latlong[1]; 
        $mapDireccion = $latlong[2]; 


        $stmt = $this->db->prepare("UPDATE users SET country = (:country), lat = (:lat), lng= (:lng) WHERE id = (:accountId)");

        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
        $stmt->bindParam(":country", $mapDireccion, PDO::PARAM_STR);
        $stmt->bindParam(":lat", $mapLat, PDO::PARAM_INT);
        $stmt->bindParam(":lng", $mapLong, PDO::PARAM_INT);
    ...
    public function setFullname($fullname)
    {
        ...
        $group_name = str_replace(' ', '-', $fullname);
            $group_name = preg_replace('/[^A-Za-z0-9\-]/', '', $group_name);
            $group_name = preg_replace('/-+/', '', $group_name);

        $stmt = $this->db->prepare("UPDATE users SET fullname = (:fullname), login = (:login) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
        $stmt->bindParam(":login", $group_name, PDO::PARAM_STR);
    ...
    public function edit2($fullname, $finalidad)
    {
        $result = array("error" => true);
        $group_name = str_replace(' ', '-', $fullname);
            $group_name = preg_replace('/[^A-Za-z0-9\-]/', '', $group_name);
        ...
        $stmt = $this->db->prepare("UPDATE users SET fullname = (:fullname), login = (:login), finalidad = (:finalidad) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
        $stmt->bindParam(":finalidad", $finalidad, PDO::PARAM_STR);
        $stmt->bindParam(":login", $group_name, PDO::PARAM_STR);
        $stmt->bindParam(":fullname", $fullname, PDO::PARAM_STR);
    ...
    public function updateCountersPlan()
    {
        $postsCount = $this->getPostsCount();
        $followersCount = $this->getFollowersCount();
    ...
    public function getPostsPlan($itemId = 0)
    {
        if ($itemId == 0) {
